...
Tests are layered: unit tests for pure logic, integration tests with temporary Git repositories and SQLite, real-process tests for compiled CLI/server, and Playwright MCP checks against real Vite/server processes. Every write test uses a new staging run or tempdir; the real reference repository is read-only and excluded from write tests.
